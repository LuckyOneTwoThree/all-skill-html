...
```bash
node {SKILL_DIR}/scripts/live.mjs
```
...
```
LOOP:
  node {SKILL_DIR}/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node {SKILL_DIR}/scripts/live-status.mjs
node {SKILL_DIR}/scripts/live-resume.mjs --id SESSION_ID
node {SKILL_DIR}/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node {SKILL_DIR}/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node {SKILL_DIR}/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node {SKILL_DIR}/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node {SKILL_DIR}/scripts/live-server.mjs stop
```
...
```bash
node {SKILL_DIR}/scripts/detect-csp.mjs
```
